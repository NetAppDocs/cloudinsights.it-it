...
NOTE: Questo Cloud Insights Collector richiede un utente ONTAP con un ruolo _filesystem-scoped_. Consulta l'AWS link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/roles-and-users.html["Ruoli e regole"] documentazione per le opzioni disponibili. Attualmente AWS supporta solo un tipo di ruolo utente con ambito filesystem, che è _fsxadmin_. Questo è il ruolo appropriato da utilizzare per il Collector Cloud Insights. All'utente dovrebbero essere assegnate anche tutte e tre le seguenti applicazioni: http, ontapi, ssh.

...
| Su Kubernetes, i miei pod Telegraf riportano il seguente errore: "Errore nell'elaborazione delle informazioni sui mountstats: Impossibile aprire il file mountstats: /Hostfs/proc/1/mountstats, errore: Open /hostfs/proc/1/mountstats: Permesso negato" | Se SELinux è abilitato e abilitato, probabilmente impedisce ai pod Telegraf di accedere al file /proc/1/mountstats sui nodi Kubernetes. Per allentare questa restrizione, modificare l'agente (`kubectl edit agent agent-monitoring-netapp`) e modificare "privileged-mode: false" in "privileged-mode: true" 

...
| Per gli snapshot creati da workload Security (CS), esiste un periodo di eliminazione/archiviazione per gli snapshot CS? | No Non è stato impostato alcun periodo di eliminazione/archiviazione per le snapshot CS. L'utente deve definire una policy di eliminazione per le snapshot CS. Fare riferimento a. link:https://library.netapp.com/ecmdocs/ECMP1196819/html/GUID-27D0E37F-5AF1-4AF9-BDEB-9A4B7AF3B4A9.html["Documentazione ONTAP"] su come impostare i criteri. 

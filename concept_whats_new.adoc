...
Sono disponibili nuove API per la sicurezza dei workload per la gestione di Data Collector (add/update/delete) e agenti. Accedere a *Admin > API Access > API Documentation* e selezionare il tipo di API _workload Security_ per ulteriori informazioni.
...
Oltre alla versione di prova iniziale di Cloud Insights, è possibile usufruire di link:concept_subscribing_to_cloud_insights.html#module-trials["Versioni di prova dei moduli"]. Ad esempio, se sei abbonato a Cloud Insights e hai monitorato lo storage e le macchine virtuali, quando Aggiungi Kubernetes al tuo ambiente, entrerai automaticamente in una prova di 30 giorni di Kubernetes Observability. L'utilizzo delle unità gestite da Kubernetes Observability non verrà conteggiato in base al tuo diritto sottoscritto fino alla fine del periodo di prova.



=== Quanto salutano i miei carichi di lavoro?

Lo stato dei workload è disponibile con una semplice occhiata alla pagina *Kubernetes > Esplora > workload*, in modo da poter vedere rapidamente quali carichi di lavoro funzionano correttamente e quali potrebbero richiedere assistenza.
...
La sicurezza del carico di lavoro include i seguenti miglioramenti:

* Gli algoritmi di rilevamento sono stati migliorati per ridurre il numero di avvisi falsi positivi.
* Le prestazioni dell'interfaccia utente sono state migliorate durante il caricamento, la visualizzazione e lo scorrimento delle pagine Forensics.
